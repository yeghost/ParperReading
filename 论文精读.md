...
MDS，每个编码块，最少n-1次算数计算，每个数据块更新，至少m操作
...
w比较大时，穷举法寻找最优柯西矩阵是不可行的，设计了一个算法，寻找构造好的柯西矩阵，GC(n,m,w),n=m时找到的是最优的，n不等于m，有些性能比最优略差
...
不是2的指数时，可以通过比较大的n’逐渐删除得到最优解

m不等于n时，假设n > m，先构造出GC(m, m, w)，然后对可能被添加的行进行排序，然后添加n-m的最小行

时间复杂度O（2<sup>2w+1</sup>）

## 6 Performance ofGCMatrices
...
能确定的166个最优柯西矩阵中，53个属于符合，性能差异最大的是GC(10,2,5)，相差7.9%，其他性能差异平均位1.78%
...
![image-20211209213518634](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211209213518634.png)

m+n>256时，GF会慢，在GF(2<sup>16</sup>)进行计算
...
本文提高编码性能,CPU缓存效率对编码性能有很大影响，提出了集中 编码调度算法来优化缓存内存的使用。XOR调度

## 1.INTRODUCTION

编码是一种频繁操作，性能对系统的整体性能至关重要，随着SSD的使用，性能对系统的整体性能至关重要，I/O代价比旋转磁盘低，从而CPU资源更有价值.XOR明显由于RS。

XOR是一个快速操作，对XOR编码，主要是内存限制。提高缓存效率的一种方法是使XOR操作对CPU缓存更友好。

传统XOR调度算法，都是直接从代码中构造的。但是XOR的顺序是灵活的，并且会显著影响缓存内存的行为。提出了三种XOR调度算法，有不同的编码性能。

传统，Data Words Guided (DWG)，Data Packets Guided (DPG)

DWG提高性能23%，26%

使用VTune Performance Analyzer测试性能
...
缓存位于CPU和内存之间，延迟只有几个到几十个CPU周期，内存访问的延迟可以是数百个周期，缓存可以减少CPU对主存的访问次数，提高性能。

cache line

缓存分为两种，instruction cache，data cache

有不同的缓存级别

cache hit，cache miss的概念
...
> capacity miss
>
> > 缓存大小有限，更大的缓存大小和更好的缓存置换算法，可以减少
>
> conflict miss
>
> > 两部分数据同时映射道同一cache line，会发生冲突遗漏

为了增加cache hit和减少cache miss,

Temporal locality，patial locality

良好的时间局部性可以减少capacity miss

良好的空间局部性可以减少compulsory miss，conflict miss

### 3.2 The Impact of CPU Cache

分析极端情况，命中率为0%，以EVENODD为例子，2个校验块，p数据列
...
![image-20211218202155498](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211218202155498.png)

性能相差较大，最优情况下是最差情况的5倍

编码实际上是一个内存限制操作，使用更高带宽的内存也是提高性能的有效方式

## 4 XOR-SCHEDULINGALGORITHMS
...
![image-20211218203148424](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211218203148424.png)

### 4.2 Conventional XOR-Scheduling Algorithm

传统的调度如图5所示

![image-20211218203922675](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211218203922675.png)
...
PPG算法的伪码表示，PPG有良好的空间局部性

### 4.3 Parity Words Guided (PWG) XOR-Scheduling

PWG调度图6

![image-20211218210120261](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211218210120261.png)

伪代码分析

反转了内部的两个循环，改善了时间局部性，因为校验块在最里面的迭代中被重复使用，但是可能会失去数据块空间局部性，因为他访问数据块比PPG的顺序性小

### 4.4 Data Packets Guided (DPG) XOR-Scheduling

上述算法，应该是逐个产生校验块，但是可以从另一个角度排序，如何使用数据块，提出了DPG的XOR算法

![image-20211218210556353](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211218210556353.png)
...
改善了数据包的局部性，在RAID-6上，m=2，k>=10，m比k小，所以数据包的局部性比校验包的局部性更重要

### 4.5 Data Words Guided (DWG) XOR-Scheduling

流程如同图8

![image-20211218211534405](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211218211534405.png)
...
1. 四层循环比较复杂
2. 实现会有很多细节，可能会掩盖循环结构
3. 算法本质不一样，编译器很难通过一个产生另一个
...
64位机器，64位linux系统，不使用复杂的性能技术，例如SIMD，或者预取
...
![image-20211219151104451](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211219151104451.png)

![image-20211219151232287](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211219151232287.png)

![image-20211219151248105](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211219151248105.png)

![image-20211219151338102](C:\Users\nty\AppData\Roaming\Typora\typora-user-images\image-20211219151338102.png)
...
研究高效的XOR调度算法，更有效的利用CPU缓存

Data Words Guided XOR-scheduling，Data Packets Guided XOR-scheduling

Data Words Guided XOR-scheduling性能明显优于传统算法，Data Packets Guided xor性能更稳定

因为编码过程中，访问的数据量要比校验块量多，所以保持数据局部性对于高性能更为关键。
